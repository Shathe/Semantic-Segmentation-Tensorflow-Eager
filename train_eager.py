...
def train(loader, model, epochs=5, batch_size=2, show_loss=False, augmenter=False, lr=None, init_lr=2e-4,
          saver=None, variables_to_optimize=None, name_best_model = 'weights/best'):
...
if __name__ == "__main__":
    ...
    model = Segception.Segception(num_classes=n_classes, weights='imagenet', input_shape=(None, None, channels))
    ...
    # Init saver
    #can use also ckpt = tfe.Checkpoint((model=model, optimizer=optimizer,learning_rate=learning_rate, global_step=global_step)
    ...
    test_acc, test_miou = get_metrics(loader, model, loader.n_classes, train=False, flip_inference=True,
                                      scales=[0.75, 1, 1.5], write_images=True)
    print('Test accuracy: ' + str(test_acc.numpy()))
    print('Test miou: ' + str(test_miou))